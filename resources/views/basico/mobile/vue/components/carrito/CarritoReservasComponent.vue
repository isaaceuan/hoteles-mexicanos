        ...
        <section v-else>
            <div class="card-body p-0">
                <section class="px-2 pb-2">
                    <div class="row">
                        <div class="col-6"><b> {{$t('carrito.llegada')}}:</b></div>
                        <div class="col-6 text-right text-capitalize">
                            {{fecha_entrada | fechaDia}}
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-6"><b> {{$t('carrito.salida')}}:</b></div>
                        <div class="col-6 text-right text-capitalize">{{fecha_salida |
                            fechaDia}}
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-6"><b> {{$t('carrito.estancia')}}:</b></div>
                            ...
                            <div class="w-100 font-weight-semibold cursor-pointer">
                                <p class="mb-1 m-0">
                                    <button
                                        ...
                                        class="btn-secondary fa fa-trash position-absolute
                                        border border-dark p-2 fa-lg bg-light text-dark"></button>
                                ...
                                <div class=" d-flex">
                                    <div class="col-7 pl-0 line-height-1">
                                        <small>{{item.tarifa.nombre}}</small><br>
                                        <small>
                    ...
                    <div class="card-body px-0 pt-2">
                        <div class="text-center px-4">
                            ...
                            <p class="text-uppercase font-12">
                                {{$t('carrito.vacio_carrito')}}
                            </p>
                        </div>
                    </div>
                    <div class="card-footer bg-acento text-light rounded-0">
                        <div class="d-flex">
...
<script>
    import {carritoEvent, botonesFlotantesEvent, cambiarMonedaEvent, totalCarritoEvent} from "../../app";
    ...
    export default {
        ...
        created() {
            ...
            //Caha los eventos de los botones flotantes del footer
            botonesFlotantesEvent.$on('siguiente', obj => {
                this.next();
            });
            botonesFlotantesEvent.$on('atras', obj => {
                this.prev();
            });
        ...
        methods: {
            getCarrito() {
                totalCarritoEvent.$emit('cargadorCarritoEvent', true);
                this.loading = true;
                this.errored = false;
                axios.post(window.url_carrito_resumen)
                    .then(response => {
                        this.carrito = response.data;
                        totalCarritoEvent.$emit('totalCarritoEvent', this.carrito.total);
                        totalCarritoEvent.$emit('cargadorCarritoEvent', false);
